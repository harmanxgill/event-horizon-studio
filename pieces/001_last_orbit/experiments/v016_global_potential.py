...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v016_global_potential.png"
...
def v016_field(
    ...
    return (rings + halo + bridge + tail + potential) * (1.0 - silhouette)
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v016_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v016 - global potential.", DEFAULT_OUTPUT)
...
def main() -> None:
    args = parse_args()
    render = phase_series if args.series else v016_rgb
    rgb = render(
        width=args.width,
        height=args.height,
        a=args.a,
