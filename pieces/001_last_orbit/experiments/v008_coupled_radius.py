...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v008_coupled_radius.png"
...
def natural_twist(order: int = 4, sharpness: float = 80.0) -> float:
    if int(order) < 1:
        raise ValueError("order must be a positive integer")
    if sharpness <= 0.0:
        raise ValueError("sharpness must be positive")
    return float(np.pi * np.sqrt(sharpness) / int(order))
...
def coupled_angle(
    ...
    return theta + float(twist) * (r - float(ring_radius))
...
def ring_field(
    ...
    if twist is None:
        twist = natural_twist(order, sharpness)

    phi1 = coupled_angle(theta1, r1, ring_radius, twist)
    phi2 = coupled_angle(theta2, r2, ring_radius, twist)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render v008 - coupled radius.")
