...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v024_fine_structure.png"
...
def wave_amplitude(rho: np.ndarray, gamma_w: float = 2.0) -> np.ndarray:
    if gamma_w < 0.0:
        raise ValueError("gamma_w must be non-negative")
    return 1.0 / (1.0 + float(gamma_w) * rho)
...
def quadrupole_wave(
    ...
    if core_w < 0.0:
        raise ValueError("core_w must be non-negative")

    argument = 2.0 * theta - wave_phase(rho, phase=phase, k_g=k_g, omega_g=omega_g)
    wave = wave_amplitude(rho, gamma_w) * np.cos(argument)
    if core_w == 0.0:
        return wave
    # Theta has no value at rho = 0, where the amplitude is largest
    return wave * (1.0 - np.exp(-((rho / float(core_w)) ** 2)))
...
def ring_field(*args, **kwargs) -> np.ndarray:
    a1, a2 = ring_components(*args, **kwargs)
    return a1 + a2
...
def velocity_field(
    ...
    return s1 * np.cos(theta1) + s2 * np.cos(theta2)
...
def v024_field(
    ...
    gamma_w: float = 2.0,
    core_w: float = 0.06,
    epsilon_w: float = 0.10,
    beta_d: float = 0.30,
    p_d: float = 3.0,
    tone: str = "exponential",
    gamma_l: float = 0.25,
    p_l: float = 1.5,
    k_l: float = 0.56,
    epsilon_b: float = 0.003,
    gamma_b: float = 0.5,
    eta_b: float = 0.5,
    k_b: float = 12.0,
    m_b: int = 5,
    ...
    d1 = doppler_factor_inspired(np.cos(theta1), beta_d=beta_d, p_d=p_d)
    d2 = doppler_factor_inspired(np.cos(theta2), beta_d=beta_d, p_d=p_d)
    rings = d1 * s1 * n1 + d2 * s2 * n2
    halo = halo_field(
        r1,
        r2,
        ring_radius=ring_radius,
    ...
    rho, theta_global = global_polar(x, y)
    wave = quadrupole_wave(
        rho,
        theta_global,
        phase=phase,
        k_g=k_g,
    ...
    if not -1.0 < epsilon_w < 1.0:
        raise ValueError("epsilon_w must lie in (-1, 1)")
    modulation = 1.0 + float(epsilon_w) * wave
    ...
    intensity = rings + halo + bridge + tail + potential
    raw = intensity * modulation * (1.0 - silhouette)
    level = compress_intensity(raw, tone=tone, gamma_l=gamma_l, p_l=p_l, k_l=k_l)

    background = mathematical_background(
        rho,
        theta_global,
        epsilon_b=epsilon_b,
        gamma_b=gamma_b,
    ...
    return level + background * (1.0 - silhouette)
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v024_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
TONES = ("exponential", "hill", "linear")


def compress_intensity(
    field: np.ndarray,
    tone: str = "exponential",
    gamma_l: float = 0.25,
    p_l: float = 1.5,
    ...
    if tone == "exponential":
        ...
        return 1.0 - np.exp(-float(gamma_l) * field)
    if p_l <= 0.0 or k_l <= 0.0:
        raise ValueError("p_l and k_l must be positive")
    powered = np.maximum(field, 0.0) ** float(p_l)
    return powered / (powered + float(k_l))


STRUCTURES = ("log", "product")
...
def fine_structure(
    ...
    if float(m_n) != int(m_n) or float(k_theta) != int(k_theta):
        # both multiply an angle that jumps at +-pi; only whole numbers keep it continuous
        raise ValueError("m_n and k_theta must be integers")
    if soft_n <= 0.0:
        raise ValueError("soft_n must be positive")

    phase = float(k_r) * r + int(m_n) * theta
    if structure == "product":
        return 1.0 + float(epsilon_n) * np.sin(phase) * np.sin(int(k_theta) * theta)
    return 1.0 + float(epsilon_n) * np.sin(phase + float(c_n) * np.log(r + float(soft_n)))


def mathematical_background(
    rho: np.ndarray,
    theta: np.ndarray,
    epsilon_b: float = 0.003,
    gamma_b: float = 0.5,
    ...
    if epsilon_b < 0.0 or gamma_b < 0.0:
        raise ValueError("epsilon_b and gamma_b must be non-negative")
    if not 0.0 <= eta_b <= 1.0:
        raise ValueError("eta_b must lie in [0, 1]")
    if float(m_b) != int(m_b):
        # a fractional m_b makes the pattern jump across the atan2 cut at theta = +-pi
        raise ValueError("m_b must be an integer")

    envelope = float(epsilon_b) * np.exp(-float(gamma_b) * rho)
    return envelope * (1.0 + float(eta_b) * np.cos(float(k_b) * rho + int(m_b) * theta))


def colour_equations(
    level: np.ndarray,
    wave: np.ndarray | float = 0.0,
    rate_r: float = 4.0,
    rate_g: float = 2.0,
    rate_b: float = 0.7,
    shift_g: float = 0.05,
    ...
    if abs(shift_g) >= 1.0 or abs(shift_b) >= 1.0:
        raise ValueError("colour shifts must lie in (-1, 1)")

    red = 1.0 - np.exp(-float(rate_r) * level)
    green = (1.0 - np.exp(-float(rate_g) * level)) * (1.0 + float(shift_g) * wave)
    blue = (1.0 - np.exp(-float(rate_b) * level)) * (1.0 + float(shift_b) * wave)
...
def tone_rgb(level: np.ndarray) -> np.ndarray:
...
def velocity_rgb(
    ...
    **kwargs,
    ...
    x, y = coordinate_grids(width, height)
    r1, r2 = radial_fields(x, y, a=a)
    theta1, theta2 = phased_angles(*angular_fields(x, y, a=a), phase)
    d1, d2 = distorted_radii(r1, r2, *angular_fields(x, y, a=a), kwargs.get("distortion", -0.30))

    s1, s2 = ring_components(d1, d2, theta1, theta2)
    signed = velocity_field(s1, s2, theta1, theta2)

    # V spans two orders of magnitude between the bridge and the outer disks,
    # so a linear scale on the maximum shows only the midpoint
    magnitude = np.abs(signed)
    scale = float(np.percentile(magnitude[magnitude > 0.0], 90.0)) or 1.0
    level = np.clip(signed / scale, -1.0, 1.0)

    approaching = np.clip(level, 0.0, 1.0)
    receding = np.clip(-level, 0.0, 1.0)

    red = approaching + 0.15 * receding
    green = 0.35 * approaching + 0.45 * receding
    blue = 0.20 * approaching + receding

    return np.round(np.dstack([red, green, blue]) * 255.0).astype(np.uint8)


def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v024 - fine structure.", DEFAULT_OUTPUT)
    ...
    parser.add_argument("--core-w", type=float, default=0.06)
    parser.add_argument("--epsilon-w", type=float, default=0.10)
    parser.add_argument("--beta-d", type=float, default=0.30)
    parser.add_argument("--p-d", type=float, default=3.0)
    parser.add_argument("--tone", choices=TONES, default="exponential")
    parser.add_argument("--gamma-l", type=float, default=0.25)
    parser.add_argument("--p-l", type=float, default=1.5)
    parser.add_argument("--k-l", type=float, default=0.56)
    parser.add_argument("--epsilon-b", type=float, default=0.003)
    parser.add_argument("--gamma-b", type=float, default=0.5)
    parser.add_argument("--eta-b", type=float, default=0.5)
    parser.add_argument("--k-b", type=float, default=12.0)
    parser.add_argument("--m-b", type=int, default=5)
    parser.add_argument("--structure", choices=STRUCTURES, default="log")
    ...
    parser.add_argument("--k-theta", type=int, default=16)
    ...
    parser.add_argument("--shift-g", type=float, default=0.05)
    parser.add_argument("--shift-b", type=float, default=0.10)
...
def main() -> None:
    ...
    rgb = render(
        ...
        c_n=args.c_n,
        soft_n=args.soft_n,
        **({} if args.velocity else {
            "rate_r": args.rate_r,
            "rate_g": args.rate_g,
            "rate_b": args.rate_b,
            "shift_g": args.shift_g,
            "shift_b": args.shift_b,
        }),
