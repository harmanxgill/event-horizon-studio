...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v005_angular_fields.png"
...
def angular_weight(theta: np.ndarray, anisotropy: float = 0.35) -> np.ndarray:
...
def ring_field(
    ...
    a1 = np.exp(-sharpness * (r1 - ring_radius) ** 2) * angular_weight(theta1, anisotropy)
    a2 = np.exp(-sharpness * (r2 - ring_radius) ** 2) * angular_weight(theta2, anisotropy)
    return a1 + a2
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render v005 - angular fields.")
    ...
    parser.add_argument("--anisotropy", type=float, default=0.35)
