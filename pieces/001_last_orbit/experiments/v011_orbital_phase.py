...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v011_orbital_phase.png"
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v011_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v011 - shared orbital phase.", DEFAULT_OUTPUT)
...
def main() -> None:
    args = parse_args()
    render = phase_series if args.series else v011_rgb
    rgb = render(
        width=args.width,
        height=args.height,
        a=args.a,
