...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v019_velocity_field.png"
...
def v019_field(
    ...
    rings = s1 + s2
    velocity = velocity_field(s1, s2, theta1, theta2)
    halo = halo_field(
        r1,
        r2,
        ring_radius=ring_radius,
    ...
    intensity = rings + halo + bridge + tail + potential + lambda_v * velocity
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v019_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v019 - orbital velocity field.", DEFAULT_OUTPUT)
    ...
    parser.add_argument("--lambda-v", type=float, default=0.08)
