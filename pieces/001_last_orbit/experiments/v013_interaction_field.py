...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v013_interaction_field.png"
...
def v013_field(
    ...
    return (rings + halo + interaction) * (1.0 - silhouette)
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v013_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v013 - binary interaction field.", DEFAULT_OUTPUT)
    ...
    parser.add_argument("--alpha-i", type=float, default=25.0)
...
def main() -> None:
    args = parse_args()
    render = phase_series if args.series else v013_rgb
    rgb = render(
        width=args.width,
        height=args.height,
        a=args.a,
