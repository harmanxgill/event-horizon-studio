...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v015_tidal_tails.png"
...
def v015_field(
    ...
    return (rings + halo + bridge + tail) * (1.0 - silhouette)
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v015_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v015 - tidal tails.", DEFAULT_OUTPUT)
...
def main() -> None:
    args = parse_args()
    render = phase_series if args.series else v015_rgb
    rgb = render(
        width=args.width,
        height=args.height,
        a=args.a,
