...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v021_nonlinear_intensity.png"
...
def quadrupole_wave(
    ...
    return wave_amplitude(rho, gamma_w) * np.cos(argument)
...
def v021_field(
    ...
    wave = quadrupole_wave(
        rho, theta_global, phase=phase, k_g=k_g, omega_g=omega_g, gamma_w=gamma_w
    )
...
def v021_rgb(
    ...
    return warm_rgb(f) if tone == "linear" else tone_rgb(f)
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v021_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v021 - nonlinear intensity.", DEFAULT_OUTPUT)
    ...
    parser.add_argument("--gamma-l", type=float, default=1.4)
