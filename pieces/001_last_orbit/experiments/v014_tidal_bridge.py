...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v014_tidal_bridge.png"
...
def v014_field(
    ...
    return (rings + halo + bridge) * (1.0 - silhouette)
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v014_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v014 - tidal bridge.", DEFAULT_OUTPUT)
...
def main() -> None:
    args = parse_args()
    render = phase_series if args.series else v014_rgb
    rgb = render(
        width=args.width,
        height=args.height,
        a=args.a,
