...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v030_final_tone.png"
...
def tone_toe(level: np.ndarray, tau: float = 1.0) -> np.ndarray:
    if tau <= 0.0:
        raise ValueError("tau must be positive")
    if tau == 1.0:
        return level
    # fixes 0 and 1; darkens the ground far more than the midtones, and keeps level ratios as powers
    return level ** float(tau)
...
def v030_rgb(
    ...
    **field_parameters,
    ...
    # every term is per-pixel, so strips give the same pixels in a fraction of the memory
    rgb = np.empty((height, width, 3), dtype=np.uint8)
    for start in range(0, height, int(strip_rows)):
        stop = min(start + int(strip_rows), height)
        level = tone_toe(v030_field(x[start:stop], y[start:stop], **field_parameters), tau)
        rgb[start:stop] = to_pixels(colour_equations(level, rate_r=rate_r, rate_g=rate_g, rate_b=rate_b))
    return rgb
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v030_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v030 - final tone.", DEFAULT_OUTPUT)
    ...
    parser.add_argument("--glow", type=float, default=0.15)
    ...
    parser.add_argument("--lambda-p", type=float, default=0.05)
    ...
    parser.add_argument("--gamma-l", type=float, default=0.23)
    ...
    parser.add_argument("--strip-rows", type=int, default=None)
...
def main() -> None:
    ...
    if series:
        ...
        rgb = v030_rgb(width, height, phase=phase, **options)
