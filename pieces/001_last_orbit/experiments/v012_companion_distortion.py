...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "v012_companion_distortion.png"
...
def phase_series(
    ...
    **kwargs,
    ...
    tiles = [v012_rgb(width, height, phase=phase, **kwargs) for phase in phases]
...
def parse_args() -> argparse.Namespace:
    parser = experiment_parser("Render v012 - companion-directed distortion.", DEFAULT_OUTPUT)
...
def main() -> None:
    args = parse_args()
    render = phase_series if args.series else v012_rgb
    rgb = render(
        width=args.width,
        height=args.height,
        a=args.a,
